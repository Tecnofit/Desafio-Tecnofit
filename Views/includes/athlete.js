$(document).ready(function(){
	$(document).on("click", "#new-athlete, .edit-athlete", function() {
		...
		if(typeof $(this).data("id") !== "undefined") {
			...
			$.ajax({
				method: "GET",
				url: localStorage.getItem("BASE_PROJECT") + "athlete/get-user",
				data: {
					id: $(this).data("id"),
					return: "JSON"
				},
	...
	$(document).on("click", ".remove-athlete", function() {
		...
		if(confirm("Tem certeza que deseja remove o Atleta " + name)) {
			$.ajax({
				method: "POST",
				url: localStorage.getItem("BASE_PROJECT") + "athlete/remove-user",
				data: {
					id: $(this).data("id")
				},
				dataType: 'json'
	...
	$(document).on("click", ".link-training", function() {
		...
		$.ajax({
			method: "GET",
			url: localStorage.getItem("BASE_PROJECT") + "athlete/get-trainings",
			data: {
				id: $(this).data("id"),
				return: "JSON"
			},
